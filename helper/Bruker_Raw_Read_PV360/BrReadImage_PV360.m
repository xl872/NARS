function [res, p, pathstr] = BrReadImage_PV360(path, params)
...
pnames = {'ACQ_size','ACQ_fov','ACQ_jobs_size','ACQ_jobs', 'NI', 'NR', 'NA', 'ACQ_phase_factor', 'ACQ_obj_order', ...
          'SW_h','NSLICES','ACQ_echo_time','ACQ_repetition_time',...
          'ACQ_ReceiverSelect','ACQ_sw_version'};
for CntFile = 1:NFiles
    ...
    %for PV360, Steve @08112022, also for line-scanning
    if params.movie == 1
        p.ACQ_size(1) = p.ACQ_jobs.value1;
%         p.ACQ_size(2) = p.ACQ_jobs.value4/(p.NI*p.NR)
        p.ACQ_size(2) = p.ACQ_jobs.value4/(p.NI*p.NR*p.NA)
%         p.ACQ_size(2) = p.ACQ_jobs.value2; %for Awake fMRI LS, @01182023
%         p.ACQ_size(2) = p.ACQ_jobs.value4/(p.NI*p.NR); %for BiLS, @08222022
        %     p.ACQ_size(2) = p.ACQ_jobs.value4/(p.NI*p.NR*p.NA); %for BiLS and UTE, @10272022
        %     p.ACQ_size(2) = p.ACQ_jobs.value4/(p.NI*p.NR*p.NA); %for BiLS and UTE, @10272022
        p.ACQ_size(3) = p.ACQ_jobs.value8;
%         p.ACQ_size(3) = p.ACQ_jobs.value2;
        p.ACQ_spatial_phase_1 =1;
        p.ACQ_spatial_phase_2 =1;
    else % ex) FLASH
        p.ACQ_size(1) = p.ACQ_jobs(1).value3;
        p.ACQ_size(2) = p.ACQ_jobs(2).value1; %for FLASH, @01192023, Avg 2 hard coding
        %     p.ACQ_size(2) = p.ACQ_jobs.value4/(p.NI*p.NR*p.NA); %for BiLS and UTE, @10272022
        %     p.ACQ_size(2) = p.ACQ_jobs.value4/(p.NI*p.NR*p.NA); %for BiLS and UTE, @10272022
        p.ACQ_size(3) = p.ACQ_jobs(1).value2;
        p.ACQ_spatial_phase_1 =1;
        p.ACQ_spatial_phase_2 =1;
%     else % ex) MGE
%         p.ACQ_size(1) = p.ACQ_jobs(1).value1/2;
%         p.ACQ_size(2) = p.ACQ_jobs(1).value1/2+p.ACQ_jobs(2).value1; %for FLASH, @01192023, Avg 2 hard coding
%         %     p.ACQ_size(2) = p.ACQ_jobs.value4/(p.NI*p.NR*p.NA); %for BiLS and UTE, @10272022
%         %     p.ACQ_size(2) = p.ACQ_jobs.value4/(p.NI*p.NR*p.NA); %for BiLS and UTE, @10272022
%         p.ACQ_size(3) = p.ACQ_jobs(1).value2;
%         p.ACQ_spatial_phase_1 =1;
...
%     p.ACQ_size(3) = p.ACQ_jobs.value2;
   %for B0 map , Steve @04252022
%     p.ACQ_size(2) = length(p.ACQ_spatial_phase_1);
%     p.ACQ_size(3) = length(p.ACQ_spatial_phase_2);
    
    datasize = p.ACQ_size(1)/2;
    numbers = [64,128, 144 ,160,192,256,320,384,448,512,576,640,704,768,832,896,960,1024, 1536,1920];
...
%    while numel(fid) ~= datasize*p.ACQ_phase_factor*p.NI*p.ACQ_size(2)/(p.NA)*p.NR
   while numel(fid) ~= datasize*p.ACQ_phase_factor*p.NI*p.ACQ_size(2)/p.ACQ_phase_factor*p.NR
...
%         else
   ...
   NReceivers = 1; %Force 1 for Proton imaging
...
%     if p.NI > 1 %Assume that Movie Loop ON!!!
% %         tempfid = reshape(fid,[params.zf(1),params.zf(2),params.zf(3),params.zf(4),p.NSLICES,p.NI/p.NSLICES, p.NR, NReceivers]);
%         nMovie = p.NI/p.NSLICES;
%         iM = 1;
%         while(iM <= nMovie)
%             for iS = 1 : p.NSLICES 
% %                 tempfid = ACQ_obj_order[k]= PVM_MbObjOrderList[i]*PVM_NMovieFrames +j;
%                   tempfid(:,:,:,:,iS,iM,:,:) =  fid(:,:,:,:,iM+nMovie*(iS-1),:,:);
%             end
%             iM=iM+1;
%         end
%         fid = tempfid;
