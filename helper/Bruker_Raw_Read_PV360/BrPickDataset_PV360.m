function [res, fileinfo] = BrPickDataset_PV360(path, return_fileinfo, requiredStatus, title, fileinfo)
...
%requiredStatus can be either 'all', 'rawdata.job0' or 'image'. With 'all', all scan
%directories are listed, with 'rawdata.job0' only those where an rawdata.job0-file exists,
%and with 'image' only those where an 2dseq-file exists. Default = rawdata.job0
...
if exist('fileinfo') == 1 & isstruct(fileinfo)
    ...
    for cnt = 1:nScans
        ...
        ScanInfo(cnt,4) = '|';
        filename{cnt} = [path,filesep,fileinfo(cnt).filename,filesep,'rawdata.job0'];
    end
else
    ReadFileinfo = 1;
...
if ReadFileinfo == 1
    for cnt = 1:numel(direc) 
        if direc(cnt).isdir & min(ismember(direc(cnt).name,numbers))==1 
            if requiredStatus(1) == 'r'
                if exist([path,filesep,direc(cnt).name,filesep,'rawdata.job0'])~=2
                    NoScan = 1;
                else
                    NoScan = 0;
                end
            ...
            if NoScan==0
                ...
                if isstruct(p)
                    params(index) = p;
                    filename{index} = [path,filesep,direc(cnt).name,filesep,'rawdata.job0'];
