...
[ROI,info]=BrikLoad([dd(1).folder,'/ROI01+orig']);
ROI_flip=flip(ROI,1);
opt.prefix=[dd(1).folder,'/ROI01bothside'];
...
load([dd(1).folder,'/MeanKSpaceZIPthr_raw0_18_cutAVE30ALLImage.mat']);
RandomN=1000;
n=size(Kall,5);
k=16;
%%
...
[Vn,infon]=BrikLoad([dd(1).folder,'/random/blur_ave_',prefix,'+orig']);

optn.OverWrite='y';
optn.prefix=[dd(1).folder,'/random/blur_aveALL_',prefix];
Vnn=[];
EN=4;%echo num
for ii=1:EN
    Vnn(:,:,:,:,ii)=Vn(:,:,:,(ii-1)*size(img_4d,4)/EN+1:(ii-1)*size(img_4d,4)/EN+size(img_4d,4)/EN-cutoff);
end
Vnn=mean(Vnn,5);
...
delete([dd(1).folder,'/random/demean_func_',prefix,'AVE+orig*'])
system(['3dDeconvolve -input ',dd(1).folder,'/random/blur_aveALL_',prefix,'+orig ' ...
    '    -nfirst 0 -polort 2 -num_stimts 1 -stim_times 1 ' ...
    '    ''1D: ',num2str(StimTime),''' ''BLOCK(',num2str(BLOCK),',1)'' ' ...
    '    -stim_label 1 L_FP -tout -fout -rout ' ...
    '    -bucket ',dd(1).folder,'/random/demean_func_',prefix,'AVE'])
...
for i=1:RandomN
    prefix=['MeanKSpaceZIP',num2str(i),'_cutAVE',num2str(k)];
    [StatV,info]=BrikLoad([dd(1).folder,'/random/demean_func_',prefix,'AVE+orig']);
...
grid off;
print(gcf,[AFNIdata,'RANDOM01.png'],'-dpng', '-r300')
exportgraphics(gcf, [AFNIdata,'RANDOM01.pdf'], 'ContentType', 'vector');
%%
